...
// free functions
    ...
    std::vector<std::string> getLoadedLibraries()
    {
        ...
        DWORD cbNeeded;
        if (EnumProcessModules(GetCurrentProcess(), handles, handles.size(), &cbNeeded)) {
            std::array<char, _MAX_PATH> szFilename;
            for (const auto& h : handles) {
                GetModuleFileNameA(handle, szFilename.data(), szFilename.size());
                std::string path(szFilename.data() 1);
