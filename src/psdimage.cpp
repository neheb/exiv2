...
void PsdImage::doWriteMetadata(BasicIo& outIo) {
  ...
  while (oldResLength > 0) {
    ...
    if ((resourceId == kPhotoshopResourceID::IPTC_NAA || resourceId > kPhotoshopResourceID::IPTC_NAA) && !iptcDone) {
      newResLength += writeIptcData(iptcData_, outIo);
      iptcDone = true;
    }

    // Write ExifInfo resource block
    else if ((resourceId == kPhotoshopResourceID::ExifInfo || resourceId > kPhotoshopResourceID::ExifInfo) &&
             !exifDone) {
    ...
    else if ((resourceId == kPhotoshopResourceID::XMPPacket || resourceId > kPhotoshopResourceID::XMPPacket) &&
             !xmpDone) {
